...
WebsocketClient::WebsocketClient()
  ...
  thread_.reset(new websocketpp::lib::thread(&IWebsocketClient::run, &client_));
}
...
void WebsocketClient::Connect(const std::string& url) {
  ...
  if (ec) {
    blog(LOG_DEBUG, "init connection error: %s", ec.message().c_str());
    return;
  }
  url_ = url;
  ...
  try {
    ...
    conn->set_open_handler(
        websocketpp::lib::bind(&WebsocketClient::OnConnectionOpen, this));
    conn->set_close_handler(
        websocketpp::lib::bind(&WebsocketClient::OnConnectionClose, this));
    conn->set_fail_handler(
        websocketpp::lib::bind(&WebsocketClient::OnConnectionFail, this));
    conn->set_message_handler(websocketpp::lib::bind(
        &WebsocketClient::OnRecvMsg, this, websocketpp::lib::placeholders::_1,
